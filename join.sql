...
SELECT genres.id, genres.name, SUM(milliseconds) AS total_length
FROM genres
JOIN tracks
  ON (tracks.genre_id = genres.id)
GROUP BY genres.id
...
SELECT genres.id, genres.name, AVG(milliseconds) AS average_length
FROM genres
JOIN tracks
  ON (tracks.genre_id = genres.id)
GROUP BY genres.id
...
SELECT albums.id, albums.title, SUM(milliseconds) AS total_length
FROM albums
JOIN tracks
...
SELECT albums.id, albums.title, AVG(milliseconds) AS average_length
FROM albums
JOIN tracks
  ON (tracks.album_id = albums.id)
GROUP BY albums.id
...
SELECT albums.id, albums.title, SUM(unit_price) AS total_price
FROM albums
JOIN tracks
  ON (tracks.album_id = albums.id)
GROUP BY albums.id
...
SELECT tracks.id, tracks.name, SUM(quantity) AS total_sold
FROM tracks
JOIN invoice_lines
